    ...
    <div className="space-y-6">
      ...
      {clients.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {clients.map((client) => (
            <div key={client.id || client._id} className="card hover:border-amber-600 transition-all duration-300 group">
              {/* رأس البطاقة */}
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-gray-700">
                ...
                <Link
                  to={`/clients/${client.id || client._id}/edit`}
                  className="p-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors"
                >
                  <Edit size={16} />
                </Link>
                <button
                  onClick={() => setDeleteConfirm(client.id || client._id)}
